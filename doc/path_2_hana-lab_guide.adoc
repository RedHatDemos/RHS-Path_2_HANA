...
----
----
[root@tower ~]$ ssh hana1.example.com
[root@hana0 ~]$ exit
----
...
We will use HANA Express for the labs as Red Hat is not allowed to provide SAP software for training purpose. So you have to register and download your own HANA Express version.
...
On http://galaxy.ansible.com[Ansible Galaxy] a lot of ready to use roles exist. Red Hat maintains the linux-system-roles, which are upstream to supported RHEL System Roles.
...
These and some other useful roles can be found here: https://galaxy.ansible.com/mk-ansible-roles/[Roles Overview]
